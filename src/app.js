...
function getSafeSessions() {
  return chatSessions.map(s => ({
    ...
    messages: s.messages.map(m => ({
      ...m,
      parts: m.parts ? m.parts.map(p => p.inlineData ? { text: '📷 [Image attached]' } : p) : undefined
    }))
  }));
}
...
async function sendChat(customParts = null){
  ...
  try {
    ...
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:streamGenerateContent?alt=sse&key=${(import.meta.env.VITE_GEMINI_API_KEY || settings.gemini_key)}`, {
      method: 'POST',
